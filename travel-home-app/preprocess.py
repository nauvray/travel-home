...
def save_images_as_npy(csv_path : str, csv_number : str, df : pd.DataFrame, image_indexes : list=None) -> None:
    '''given a data frame containing images, save images matching the list of indexes (npy files with uint8 data type)'''

    if image_indexes is None:
        image_indexes = list(range(df.shape[0]))

    for i in image_indexes:
        image = Image.open(BytesIO(bytes.fromhex(df.data[i])))

        ## concatenate image name with the csv number (separated with '__')
        # npy_img_name = df.img[i].strip('.jpg') + SEPARATOR + csv_number + '.npy'
        npy_img_name = df.img[i].strip('.jpg') + '.npy'
        ...
        folder = os.path.join(csv_path, csv_number)
        if not os.path.isdir(folder):
            os.mkdir(folder)

        npy_file_path = os.path.join(folder, npy_img_name)
