            ...
            for index, file in enumerate(files):
                if index + 1 >= nb_files * 0.7:
                    destination_folder = os.path.join(
                        val_images_path, file.path.split("/")[-2]
                    )
                ...
                if index + 1 < nb_files * 0.7:
                    destination_folder = os.path.join(
                        train_images_path, file.path.split("/")[-2]
                    )
                    if not os.path.isdir(destination_folder):
                        os.mkdir(destination_folder)
                    destination = os.path.join(destination_folder, file.name)
...
def prepare_input_train(data_dir: str):
    image_datasets = {
        x: datasets.DatasetFolder(
            ...
            loader=npy_loader,
            extensions=[".npy"],
            transform=images_transformer(x),
        )
        for x in ["train", "val"]
    }
    dataloaders = {
...
def train_model(dataloaders, model, num_epochs):
    ...
    dataset_sizes = {
        x: sum(
            [len(files) for r, d, files in os.walk(os.path.join(WORKING_DIR, "npy", x))]
        )
        for x in ["train", "val"]
    }
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            # to be deleted
            print(
                f"{running_corrects} are correct {running_corrects.double()} also, loss is : {running_loss}"
            )
